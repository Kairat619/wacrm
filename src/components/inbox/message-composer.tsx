...
export function MessageComposer({
  ...
  // Voice recording state. The browser's native MediaRecorder captures
  // whatever container it's good at (WebM/Opus on Chrome, Ogg/Opus on
  // Firefox, MP4/AAC on Safari); non-Ogg takes are converted to
  // WhatsApp-voice-note Ogg/Opus by /api/media/voice-note before upload.
  const [recording, setRecording] = useState(false);
  const [recordSeconds, setRecordSeconds] = useState(0);
  const recorderRef = useRef<MediaRecorder | null>(null);
  // Mic stream is held separately so we can release tracks even if the
  // recorder never reaches a live start — a leaked getUserMedia lock
  // blocks every other app until the tab closes.
  ...
  useEffect(() => {
    return () => {
      ...
      // stop() fires onstop, which releases the mic tracks. Tracks are
      // also stopped directly below in case the recorder never started.
      try {
        if (recorderRef.current && recorderRef.current.state !== 'inactive') {
          recorderRef.current.stop();
        }
      } catch {
        // Recorder already torn down — nothing to release here.
      }
  ...
  const finalizeRecording = useCallback(
    async (blob: Blob, mimeType: string) => {
      ...
      if (blob.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
  ...
  const startRecording = useCallback(async () => {
    ...
    // MediaRecorder rides on getUserMedia; both only exist in secure
    ...
    // Acquire the mic first so permission failures never touch recorder
    // state. describeMicError maps NotAllowed/NotFound/NotReadable/…
    ...
    recorder.onstop = () => {
      // Runs on both stop AND cancel — release the mic exactly once here.
      streamRef.current?.getTracks().forEach((track) => track.stop());
      streamRef.current = null;
      ...
      chunksRef.current = [];
      recorderRef.current = null;
      if (cancelledRef.current) return; // discard the take
      void finalizeRecording(new Blob(chunks, { type }), type);
    };

    // No timeslice: recordings are capped at 5 min and stop() flushes
    // everything in one event.
    recorder.start();
    setRecording(true);
    setRecordSeconds(0);
  ...
  // Release the mic when the recorder can't do it for us (never started).
  ...
  const stopRecording = useCallback(() => {
    ...
    setRecording(false);
    if (recorderRef.current && recorderRef.current.state !== 'inactive') {
      recorderRef.current.stop(); // onstop finalizes + releases the mic
    } else {
      releaseStream();
    }
